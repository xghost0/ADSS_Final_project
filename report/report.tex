...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Advanced System Security -- Serverless Credential Theft with eBPF Uprobes}}
...
\begin{titlepage}
    ...
    {\large Advanced System Security (ADSS) -- Final Project\\[2cm]}
    ...
    {\large
    \begin{tabular}{ll}
        \textbf{Authors:} & Alice Drifte \\
                           & Kevin Illi \\
                           & Stefan Hartmann \\[0.5em]
        \textbf{Date:}    & \today \\
        \textbf{Module:}  & Advanced System Security (ADSS) \\
    \end{tabular}
    }
...
\midrule
\texttt{pam\_params\_map} & Thread ID & \texttt{char **} (authtok pointer) & Pass authtok address from entry to return \\
...
probe and the return probe, since the function execution overwrites them. By storing
...
\texttt{const char **authtok}, i.e.\ a pointer to a pointer. At entry, we capture the
